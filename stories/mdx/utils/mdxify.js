...
function getMDX(args) {
  ...
  if (args.args.children) {
    ...
    return "<" + componentName + getAttributes(args.args) + "/>";
  }
}
...
function useMdxSerializer(componentName, componentArgs) {
  ...
  if (compiledSource) {
    // the content is already processed
...
function Wrapper({ context, children }) {
  ...
  if (context && context.component && context.component.render) {
    ...
    mdx = `<Zoom maxWidth='${parseInt(pageSize.width)}' width='${parseInt(
      pageSize.width
    )}' maxHeight='${parseInt(pageSize.height)}' height='${parseInt(
      pageSize.height
    )}' sx={{ maxWidth: '100vw', maxHeight: '100vh' }}><div style={{ border: '2px solid grey', padding: '0px' }}>${getMDX(
      context
    )}</div></Zoom>`;
  } else {
    mdx = getMDX(context);
...
export { useMdxSerializer, getMDX, Wrapper, WrapTheme, WrapMDX };
