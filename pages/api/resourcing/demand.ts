...
function cleanFields(userData: { [x: string]: any }) {
...
async function getUsersFromAzureAD(token: string) {
...
const extractCodeForRedis = (jsonData: string) => {
  // Parse the JSON data
...
const extractEmailsForRedis = (jsonData: string) => {
  // Parse the JSON data
  const data = JSON.parse(jsonData);

  // Extract the 'mail' field from each object
  const emails = data
    .map((item: { mail: string }) => item.mail)
    .filter((mail: string) => mail);

  // Return the array of emails
  return emails;
};

const extractCodeForRedi2s = (jsonData: string) => {
  // Parse the JSON data
  const data = JSON.parse(jsonData);
...
function mergeApiAndRedisResults(apiResults: string, redisResults: any[]) {
...
export default async function handler(req: Request, res: Response) {
  ...
  if (req.method === 'POST') {
    ...
    const data = await new Promise((resolve, reject) => {
      ...
      form.parse(req, (err: any, _fields: any, files: unknown) => {
        if (err) {
          return reject(err);
        }
        resolve(files);
    ...
    if (req.query.demand === 'true') {
      try {
        ...
        const stripPrefix = (reservation: string) => {
        ...
        keysToDelete &&
          keysToDelete.forEach(async (key: string) => {
            try {
