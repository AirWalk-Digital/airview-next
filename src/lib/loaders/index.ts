...
function escapeInvalidXmlChars(xmlContent: string) {
 ...
 */
export function parseExcelXml(xmlContent: string) {
  ...
  return parseStringPromise(escapedXmlContent, {
    ...
    // Extract headers
    const headers = rows[0].Cell.map((cell) =>
      cell.Data ? cell.Data._ || cell.Data : ''
    );
...
export function expandResourceData(data, userData) {
    ...
    return groupedData.map((item) => {
      ...
      if (userData) {
        fieldsToAdd.forEach((field) => {
          ...
          const camelCaseField = field.replace(/_([a-z])/g, (match, p1) =>
            p1.toUpperCase()
          );
  ...
  let augmentedData = addFieldsToData(
    groupedData,
    fieldsToAdd,
    replyNameToUserData
  );
  ...
  const groupedMonthData = augmentedData.map((item) => {
    ...
    item.jobs.forEach((job) => {
      ...
      if (
        typeof job.hypo_dd === 'number' ||
        (typeof job.hypo_dd === 'string' && !isNaN(parseInt(job.hypo_dd, 10)))
      ) {
        hypoDd = parseInt(job.hypo_dd, 10);
      }

      if (
        typeof job.work_dd === 'number' ||
        (typeof job.work_dd === 'string' && !isNaN(parseInt(job.work_dd, 10)))
      ) {
      ...
      if (!jobsByMonth[month]) {
        ...
        if (hypoDd !== null) {
          jobsByMonth[month].days_hypo = Math.max(
            jobsByMonth[month].days_hypo,
            hypoDd
          );
        }
        if (workDd !== null) {
          jobsByMonth[month].days_working = Math.max(
            jobsByMonth[month].days_working,
            workDd
          );
        }
      }
    ...
    const filteredJobsByMonth = Object.values(jobsByMonth).filter(
      (monthData) => monthData.days_allocated > 0 || monthData.days_forecast > 0
    );
...
export function calculateDemand(jsonData) {
  ...
  // console.log('lib/loaders/calulateDemand: ', JSON.stringify(nobUsers))
    ...
    user.jobs.forEach((booking) => {
      booking.jobs.forEach((job) => {
        ...
        const userDetails = {
          ...
          days_forecast: booking.days_forecast,
          days_hypo: booking.days_hypo,
          days_working: booking.days_working,
        };

        if (!adjustedPivotedData[customer]) adjustedPivotedData[customer] = {};
        if (!adjustedPivotedData[customer][code])
          adjustedPivotedData[customer][code] = {};
        if (!adjustedPivotedData[customer][code][description])
          adjustedPivotedData[customer][code][description] = {};
        if (!adjustedPivotedData[customer][code][description][month])
          adjustedPivotedData[customer][code][description][month] = [];
        adjustedPivotedData[customer][code][description][month].push(
          userDetails
        );
      });
    });
  ...
  function extractRoleAndGrade(name) {
    ...
    try {
      ...
      console.log('extractRoleAndGrade:Error: ', name);
    }
...
export async function timesheetPortalHolidays(clientID, clientSecret) {
  ...
  const threeMonths = new Date(new Date().setMonth(new Date().getMonth() + 3))
    .toISOString()
    .split('T')[0];
  ...
  let response = await fetch(
    'https://api-rest.timesheetportal.com/oauth/authorise',
    {
      method: 'POST',
      headers: {
  ...
  response = await fetch(
    'https://api-rest.timesheetportal.com/leavebookings/reports/bookings',
    {
      method: 'POST',
      headers,
  ...
  if (response.status != 200) {
    console.error(
      'loaders/timesheetPortalHolidays:response:ERROR: ',
...
function groupLeaveData(data) {
  ...
  data.slice(1).forEach((row) => {
    ...
    const leaveMonth = `${leaveStartDate.getFullYear()}-${`0${
    ...
    const monthEntry = groupedData[employeeName].holidays.find(
      (entry) => entry.month === leaveMonth
    );
    if (monthEntry) {
      monthEntry.workingDays += workingDays;
    } else {
      groupedData[employeeName].holidays.push({
        month: leaveMonth,
        workingDays,
      });
    }
...
export function combineResourcesWithHolidays(dataArray, holidaysObject) {
  return dataArray.map((resource) => {
    ...
    const { displayName } = newResource;
    if (holidaysObject[displayName]) {
      holidaysObject[displayName].holidays.forEach((holiday) => {
        const jobForHolidayMonthExists = newResource.jobs.some((job) =>
          job.month.startsWith(holiday.month)
        );
      ...
      newResource.jobs.forEach((job) => {
        const holidayEntry = holidaysObject[displayName].holidays.find(
          (holiday) => holiday.month === job.month.slice(0, 7)
        );
        if (holidayEntry) {
