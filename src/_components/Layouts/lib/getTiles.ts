...
import { getAllFiles, getBranchSha, getFileContent } from '@/lib/Github';
...
export async function getTiles(config: ContentItem): Promise<{
  ...
  let cachedContent;
  try {
    logger.debug({ cacheKey, msg: 'loading cachedContent' });
  ...
  try {
    ...
    logger.error({ msg: '[Cache][FAIL]', cacheKey, error: err });
  }
  return { loading, error, tiles: cachedContent as Tile[] | null };
}

export async function getContent(siteConfig: ContentItem) {
  const branchSha = await getBranchSha(
    siteConfig.owner,
    siteConfig.repo,
    siteConfig.branch,
