...
function groupBy(data, nameKey, resourceKey, nobodyKey) {
  ...
  data.forEach((item) => {
    ...
    const {
      [toSnakeCase(nameKey)]: _,
      [toSnakeCase(resourceKey)]: __,
      [toSnakeCase(nobodyKey)]: ___,
      tbd: ____,
      status: _____,
      ...jobDetails
    } = cleanItem;
...
function groupLeaveData(data) {
  ...
  data.slice(1).forEach((row) => {
    ...
    const leaveMonth = `${leaveStartDate.getFullYear()}-${(
      "0" +
    ...
    let monthEntry = groupedData[employeeName].holidays.find(
  ...
  return dataArray.map((resource) => {
    ...
    if (holidaysObject[displayName]) {
      holidaysObject[displayName].holidays.forEach((holiday) => {
        let jobForHolidayMonthExists = newResource.jobs.some((job) =>
      ...
      newResource.jobs.forEach((job) => {
        let holidayEntry = holidaysObject[displayName].holidays.find(
