...
export function expandResourceData(data, userData) {
    ...
    const augmentedData = addFieldsToData(groupedData, fieldsToAdd, replyNameToUserData);
...
//   const jsonData = JSON.parse(fs.readFileSync(jsonFilePath, 'utf8'));
  ...
  // Process 'name' field to extract 'role' and 'grade'
    ...
    Object.values(codes).forEach(descriptions => {
      Object.values(descriptions).forEach(months => {
        Object.values(months).forEach(users => {
          users.forEach(user => {
            const { role, grade } = extractRoleAndGrade(user.name);
            user.role = role;
            user.grade = grade;
            delete user.name;
          });
