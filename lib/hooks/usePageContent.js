...
export function usePageContent(initialContent, initialContext) {
  ...
  if (initialContext.menu && initialContext.menu.collection) {
    ...
    console.debug('usePageContent:collection: ', siteConfig.content[siteConfig.content[initialContext.menu.collection]])
    return siteConfig.content[siteConfig.content[initialContext.menu.collection]]
  } else {
  ...
  function loadContent(context, content) {
    ...
    console.debug("usePageContent:loadContent:content: ", content.substring(0, 100));

    if (content && context && context.file && context.file.endsWith(".md")) { // load normal markdown files
    ...
    } else if (content && context && context.file && context.file.endsWith(".mdx")) { // load MDX files
      setContentSource('api');
      ...
      // }
      setPageContent({ content: mdxContent, frontmatter: frontmatter });
    } else if (content && context && context.file && context.file.endsWith(".etherpad")) { // load etherpad files
        ...
        if (response.ok) {
          ...
          if (url && url.endsWith(".md")) { // load normal markdown files
            ...
            if (frontmatter.external_repo || frontmatter.external) {
              ...
              const { mdxContent: mdxExtContent, frontmatter: extFrontmatter } = useMDX(extContent, 'md');
          ...
          throw new Error("usePageContent:handleContentChange:Error fetching files");
        }
      } catch (error) {
