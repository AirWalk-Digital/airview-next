...
export function usePageContent(initialContent, initialContext) {
  ...
  useEffect(() => { // process edit and branches in the querystring.
    ...
    if (editFromQuery) { // set the edit mode from the query parameter ?edit=true
      console.debug("usePageContent:queryEdit: ", editFromQuery);
      loadfiles = true
      setEditMode(true);
    } 

    if (queryBranch && context.branch != queryBranch) { // set the branch from the query parameter ?branch=
  ...
  function loadContent(context, content) {
    ...
    if (content && context && context.file && context.file.endsWith(".md")) {
      ...
      // console.log('usePageContent:loadContent:content: ', content.substring(0, 100) );
  ...
  const handlePageReset = async () => {
    ...
    await dispatch(setBranch(initialContext));
      ...
      if (url && url.endsWith(".etherpad")) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            console.debug("usePageContent:handleContentChange:data: ", data.substring(0, 100));
            ...
            if (url && url.endsWith(".md")) {
              ...
              if (frontmatter.external_repo || frontmatter.external) {
                ...
                console.log('usePageContent:handleContentChange:extContent: ', extContent)
                  const {
