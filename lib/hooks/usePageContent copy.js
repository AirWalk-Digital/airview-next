...
export function usePageContent(initialContent, initialFile, initialMenuStructure, collection, initialContext) {
  ...
  useEffect(() => {
    ...
    console.debug("usePageContent:queryBranch: ", queryBranch);
    console.debug("usePageContent:editFromQuery: ", editFromQuery);

    if (editFromQuery) {
      setEditMode(true);
    } // set the edit mode from the query parameter ?edit=true

    if (context.branch != initialContext.branch) {
      const newContext = { ...context, branch: queryBranch };
      console.debug("usePageContent:newContext: ", newContext);
  ...
  const handlePageReset = () => {
    ...
    console.log('usePageContent:handlePageReset:initialContext: ', initialContext)
  ...
  const handleContentChange = async (url, relative) => {
    ...
    if (relative===true) { // it's a relative page, so set the values
      setQueryParams(url)
      setRelPage(url);
    }
    ...
    if (url && url.endsWith(".etherpad")) {
      ...
      console.debug('usePageContent:handleContentChange:reduxCollection: ', reduxCollection)
      ...
      try {
        ...
        if (response.ok) {
          ...
          // const newContext = { ...context, file: url, ...collectionName(url, collection) };
          ...
          if (url && url.endsWith(".md")) { // load normal markdown files
            setContentSource('api');

            const { mdxContent, frontmatter } = loadMDX(data, 'md');
            if (frontmatter.external_repo || frontmatter.external) {
              // console.log('loading external content : ', newContext, frontmatter)
              // await githubExternalContent(frontmatter, newContext);
              const { newContent: extContent, newContext: extContext } = await githubExternal(frontmatter, newContext);
              // console.log('handleContentChange:extContent: ', extContent)

              const { mdxContent: mdxExtContent, frontmatter: extFrontmatter } = loadMDX(extContent, 'md');
              // console.log('handleContentChange:mdxExtContent: ', mdxExtContent)

              if (extContext && !deepEqual(context, extContext)) {
                setContext(extContext);
              }
              setPageContent({ content: mdxExtContent, frontmatter: extFrontmatter });
          ...
          } else if (url && url.endsWith(".mdx")) { // load MDX files
            setContentSource('api');
            const { mdxContent, frontmatter } = loadMDX(data, 'mdx');
          ...
          } else if (url && url.endsWith(".etherpad")) { // load etherpad files
            loadPad(url);
          }
        ...
        console.error("usePageContent:handleContentChange:Error fetching files: ", error);
      }
    }
  }
  };
